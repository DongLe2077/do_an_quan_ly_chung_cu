...
export default function SuCoPage() {
  ...
  const fetchResidentInfo = async () => {
    try {
      const res = await residentService.getByUser(user.user_id);
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      const currentFileList = values.images;
      ...
      const payload = { 
        ...
        apartment_id: values.apartment_id || cuDanInfo?.apartment_id 
      };

      if (editingRecord) {
        await incidentService.update(editingRecord.incident_id, payload);
        message.success('Cập nhật sự cố thành công');
      } else {
        await incidentService.create({ ...payload, reporter_id: user?.user_id });
  ...
  const handleXuLy = async (values) => {
    try {
      await incidentService.xuLy(editingRecord.incident_id, values);
  ...
  const columns = [
    ...
    { title: 'Tên sự cố', dataIndex: 'title', key: 'title', ellipsis: true },
    {
      title: 'Ảnh', dataIndex: 'images', key: 'images', width: 100,
      render: (val) => {
    ...
    { title: 'Phòng', dataIndex: 'apartment_number', key: 'apartment_number', width: 90 },
    { title: 'Người báo', dataIndex: 'reporter_name', key: 'reporter_name', width: 120 },
    {
      title: 'Ngày báo', dataIndex: 'report_date', key: 'report_date', width: 110,
    ...
    { title: 'Người xử lý', dataIndex: 'handler', key: 'handler', width: 120 },
    {
          ...
          {isAdmin ? (
            <>
              ...
              <Button type="link" size="small" icon={<EditOutlined />} onClick={() => {
                ...
                setFileList(formattedRecord.images);
                setModalOpen(true);
              }}>Sửa</Button>
              <Popconfirm title="Xác nhận xóa sự cố?" onConfirm={() => handleDelete(record.incident_id)} okText="Xóa" cancelText="Hủy">
      ...
      {filteredData.length > 0 ? (
        <Row gutter={[20, 20]}>
          {filteredData.map((item) => (
            <Col key={item.key || item.incident_id} xs={24} sm={24} md={12} lg={12} xl={8} xxl={8}>
              <Card 
                className="incident-card-premium"
                variant="borderless"
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                  <div className={`status-badge-modern ${getStatusGlow(item.status)}`}>
                    {getStatusIcon(item.status)}
                    <span>{item.status.toUpperCase()}</span>
                  </div>
                  <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                    {item.report_date ? new Date(item.report_date).toLocaleDateString('vi-VN') : '-'}
                  </span>
                </div>
                ...
                <div style={{ padding: '0 16px 16px 16px' }}>
                  <h3 className="incident-title" style={{ marginTop: 12 }}>{item.title}</h3>
                  <p className="incident-desc">{item.description}</p>
                  
                    <div className="incident-meta">
                      <div className="meta-item">
                        <HomeOutlined />
                        <span>Phòng {item.apartment_number}</span>
                      </div>
                      {item.handler && (
                        <div className="meta-item">
                          <UserOutlined />
                          <span>Xử lý bởi: <b>{item.handler}</b></span>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  {item.status === 'Chờ duyệt' && (
                    <div style={{ padding: '0 16px 16px', display: 'flex', gap: 8, borderTop: '1px solid #f0f0f0', paddingTop: 12 }}>
                      ...
                      <Popconfirm 
                        ...
                        onConfirm={() => handleDelete(item.incident_id)}
                        okText="Hủy báo cáo" 
                        cancelText="Quay lại"
    ...
    <div>
      ...
      <Modal title={editingRecord ? 'Sửa sự cố' : 'Báo cáo sự cố mới'} open={modalOpen}
        ...
        <Form form={form} layout="vertical" onFinish={handleSubmit}>
          ...
          <Form.Item name="description" label="Mô tả chi tiết" rules={[{ required: true, message: 'Nhập mô tả' }]}>
          ...
          {isAdmin ? (
            ...
            cuDanInfo?.apartment_id && (
              <Form.Item name="apartment_id" label="Phòng" initialValue={cuDanInfo.apartment_id} style={{ display: 'none' }}>
                <Input />
              </Form.Item>
            )
          )}
