...
export default function ChiSoDichVuPage() {
  ...
  const fetchDichVu = async () => {
    try { const res = await serviceService.getAll(); setDichVuList(res.data?.data || []); } catch {}
  };

  const fetchHoaDon = async () => {
    try { const res = await invoiceService.getAll(); setHoaDonList(res.data?.data || []); } catch {}
  };
  ...
  const availableMonths = useMemo(() => {
    ...
    data.forEach(item => {
      if (item.billing_month) months.add(item.billing_month);
      else if (item.reading_date) {
  ...
  const availableRooms = useMemo(() => {
    const rooms = new Set();
    data.forEach(item => {
      if (item.apartment_number) rooms.add(item.apartment_number);
    });
  ...
  const filteredData = useMemo(() => {
    ...
    if (filterMonth) {
      result = result.filter(item => {
        if (item.billing_month === filterMonth) return true;
    ...
    if (filterRoom) {
      result = result.filter(item => (item.apartment_number || 'Chưa gán phòng') === filterRoom);
    }
    return result;
  ...
  const groupedByRoom = useMemo(() => {
    ...
    filteredData.forEach(item => {
      const key = item.apartment_number || 'Chưa gán phòng';
  ...
  const handleSubmit = async (values) => {
    try {
      const payload = {
        ...values,
        reading_date: values.reading_date ? values.reading_date.format('YYYY-MM-DD') : undefined,
      };
      if (editingRecord) {
        await serviceReadingService.update(editingRecord.reading_id, payload);
        message.success('Cập nhật chỉ số thành công');
      } else {
        await serviceReadingService.create(payload);
  ...
  const handleDelete = async (reading_id) => {
    try {
      await serviceReadingService.delete(reading_id);
  ...
  const handleDVChange = (serviceId) => {
    const dv = dichVuList.find(d => d.service_id === serviceId);
    setSelectedDVType(dv?.service_type || null);
  };

  const openEdit = (record) => {
    setEditingRecord(record);
    handleDVChange(record.service_id);
    form.setFieldsValue({
      ...record,
      reading_date: record.reading_date ? dayjs(record.reading_date) : undefined,
    });
    setModalOpen(true);
  };
  ...
  const calcThanhTien = (item) => {
    ...
    if (item.current_reading != null && item.previous_reading != null) {
      const used = item.current_reading - item.previous_reading;
      return used > 0 ? used * donGia : 0;
    }
    return (item.quantity || 1) * donGia;
  };
    ...
    <div className="page-fade-in">
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          {roomKeys.map(roomName => {
            ...
            const thangThu = items[0]?.billing_month || '';
            ...
            return (
              <div className="card" key={roomName} style={{ padding: 0, overflow: 'hidden' }}>
                ...
                <div style={{ padding: '8px 16px' }}>
                  {items.map((item, idx) => {
                    const dvInfo = getIconForDV(item.service_name);
                    const isChiSo = item.current_reading != null && item.previous_reading != null;
                    const tieuThu = isChiSo ? item.current_reading - item.previous_reading : null;
                    const thanhTien = calcThanhTien(item);

                    return (
                      <div key={item.reading_id} style={{
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ fontSize: 12, color: 'var(--text-muted)', display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                            {isChiSo ? (
                              <>
                                <span>CS cũ: <strong style={{ color: 'var(--text-secondary)' }}>{item.previous_reading}</strong></span>
                                <span>→</span>
                                <span>CS mới: <strong style={{ color: dvInfo.color }}>{item.current_reading}</strong></span>
                                <span style={{
                                  background: dvInfo.bg, color: dvInfo.color,
                                  padding: '1px 8px', borderRadius: 6, fontWeight: 700, fontSize: 11
                                }}>
                                  Tiêu thụ: {tieuThu} {item.unit || ''}
                                </span>
                              </>
                            ) : (
                              <span>Số lượng: <strong style={{ color: 'var(--text-secondary)' }}>{item.quantity || 1}</strong></span>
                            )}
                            {item.reading_date && (
                              <span>• {new Date(item.reading_date).toLocaleDateString('vi-VN')}</span>
                            )}
                          </div>
                        </div>
                        ...
                        <div style={{ textAlign: 'right', flexShrink: 0 }}>
                          ...
                          <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                            ĐG: {formatCurrency(item.unit_price)}
                          </div>
                        </div>
                        ...
                        <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
                          <button
                            ...
                            title="Sửa"
                          >✏️</button>
                          <Popconfirm title="Xác nhận xóa bản ghi này?" onConfirm={() => handleDelete(item.reading_id)} okText="Xóa" cancelText="Hủy">
                            <button
                              className="btn-ghost"
                              style={{
                                width: 32, height: 32, borderRadius: 8, padding: 0,
      ...
      <Modal title={editingRecord ? 'Sửa chỉ số' : 'Thêm chỉ số mới'} open={modalOpen}
        ...
        <Form form={form} layout="vertical" onFinish={handleSubmit}>
          <Form.Item name="service_id" label="Dịch vụ" rules={[{ required: true, message: 'Chọn dịch vụ' }]}>
            <Select placeholder="Chọn dịch vụ" onChange={handleDVChange}
              options={dichVuList.map(d => ({ label: `${d.service_name} (${d.service_type === 1 ? 'Theo chỉ số' : 'Cố định'})`, value: d.service_id }))} />
          </Form.Item>
          <Form.Item name="invoice_id" label="Hóa đơn">
            <Select allowClear placeholder="Liên kết hóa đơn"
              options={hoaDonList.map(h => ({ label: `Phòng ${h.apartment_number || '?'} - ${h.billing_month || ''} (${h.status})`, value: h.invoice_id }))} />
          </Form.Item>
          {(selectedDVType === 1 || selectedDVType === null) && (
            <>
              <Form.Item name="previous_reading" label="Chỉ số lần ghi trước">
                <InputNumber min={0} style={{ width: '100%' }} placeholder="Nhập chỉ số cũ" />
              </Form.Item>
              <Form.Item name="current_reading" label="Chỉ số hiện tại">
          ...
          <Form.Item name="reading_date" label="Ngày ghi">
