...
export default function NguoiDungPage() {
  ...
  const handleSubmit = async (values) => {
    try {
      if (editingRecord) {
        await userService.update(editingRecord.user_id, values);
  ...
  const handleSetRole = async (values) => {
    try {
      await userService.setRole(editingRecord.user_id, values);
  ...
  const handleToggleStatus = async (record) => {
    const newStatus = record.status === 'Hoạt động' ? 'Bị khóa' : 'Hoạt động';
    try {
      await userService.update(record.user_id, { status: newStatus });
  ...
  const filteredData = useMemo(() => {
    return data.filter(item => {
      const matchSearch = !searchText ||
        ...
        item.user_id?.toLowerCase().includes(searchText.toLowerCase());
  ...
  const stats = useMemo(() => ({
    total: data.length,
    active: data.filter(d => d.status === 'Hoạt động').length,
    locked: data.filter(d => d.status === 'Bị khóa').length,
    admins: data.filter(d => d.role === 'admin').length,
    cudan: data.filter(d => d.role === 'cudan').length,
    kythuat: data.filter(d => d.role === 'kythuat').length,
  }), [data]);
    ...
    <div>
        ...
        <table className="data-table">
          ...
          <tbody>
            {paginatedData.length > 0 ? paginatedData.map((item) => {
              ...
              const bgColor = getAvatarColor(item.user_id);
              const role = roleConfig[item.role] || roleConfig.cudan;
              const status = statusConfig[item.status] || statusConfig['Hoạt động'];
              return (
                <tr key={item.user_id}>
                  <td>
                    <div className="table-row-item">
                      ...
                      <div className="table-row-info">
                        <h4>{item.username}</h4>
                        <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>ID: {item.user_id}</span>
                      </div>
                    </div>
                  </td>
                  ...
                  <td style={{ textAlign: 'center' }}>
                    <div style={{ display: 'flex', gap: 4, justifyContent: 'center' }}>
                      ...
                      <button
                        ...
                        title={item.status === 'Hoạt động' ? 'Khóa tài khoản' : 'Kích hoạt tài khoản'}
                        style={{ color: item.status === 'Hoạt động' ? 'var(--warning)' : 'var(--success)' }}
                      >
                        {item.status === 'Hoạt động' ? '🔒' : '🔓'}
                      </button>
                      <button className="btn-ghost" onClick={() => handleDelete(item.user_id)}
        ...
        <div className="card">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {Object.entries(roleConfig).map(([key, cfg]) => {
              const count = data.filter(d => d.role === key).length;
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleSubmit}>
          ...
          {!editingRecord && (
            <Form.Item name="role" label="Vai trò" initialValue="cudan">
              <Select options={[
        ...
        {editingRecord && (
          <div style={{
            ...
            <div style={{
              ...
              background: getAvatarColor(editingRecord.user_id),
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              color: 'white', fontSize: 15, fontWeight: 700
            }}>
              {getInitials(editingRecord.username)}
            </div>
            <div>
              <div style={{ fontWeight: 600, fontSize: 15 }}>{editingRecord.username}</div>
              <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                Vai trò hiện tại: {roleConfig[editingRecord.role]?.label || editingRecord.role}
              </div>
            </div>
          </div>
        )}
